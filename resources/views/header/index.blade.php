        ...
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <a href="{{url('master-data/header/create')}}" type="button" class="btn btn-inverse-primary btn-rounded btn-icon">Tambah
                        Header</a>
                        ...
                        <table id="order-listing" class="table table-striped" id="table1">
                            ...
                            @foreach ($header as $item)
                                ...
                                <td>{{ $loop->iteration }}</td>
                                    ...
                                    <td>{{ $item->konten }}</td>
                                    <td>
                                    <a href="{{url('/master-data/header/'.$item->id).'/edit'}}">
                                            <button type="button" class="btn btn-social-icon btn-inverse-success btn-rounded">
                                                <i class="ti-pencil-alt"></i>
                                            </button>
                                        </a>
                                        <form method="POST" action="{{url('/master-data/header/' . $item->id)}}" enctype="multipart/form-data">
                                            @csrf
                                            @method("DELETE")
                                        ...
                                        <a href="{{url('/master-data/header/'.$item->id)}}">
                                            <button type="button" class="btn btn-social-icon btn-inverse-success btn-rounded">
                                                Show
                                            </button>
                                        </a>
